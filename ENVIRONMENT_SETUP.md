...
### 22.2 タスクセットは公開 4 タスクではない

ログに `[evaluate] タスク用途: 3_Omni` とある。ローカルで使っている
公開 4 タスクとは別のセットで採点されるので、4 タスクの成功率を
そのままスコアの予測値として扱わないこと。

### 22.3 スコアは重み付き複合値

返ってきた 0.043 は成功率そのものではない。8 エピソードから k/8 の形で
0.043 は作れない。README にあるとおり採点用の重み設定は配布されていないため、
ローカルでは成功率と衝突・軌跡の指標を個別に見るしかない。
