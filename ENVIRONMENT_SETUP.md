...
### 34.5 loss が答えを出した: 解像度は「効かない」ことが確認された

§34.4 で挙げた 2 分岐のうち、**どちらでもない**結果だった。

| step | lowres（128） | §27（256） | 差 |
|---|---|---|---|
| 100 | 0.930 | 0.910 | +2.2% |
| 1,000 | 0.591 | 0.581 | +1.7% |
| 2,000 | 0.308 | 0.303 | +1.7% |

#### 画素数を 1/4 にしても loss は 2% しか変わらない

これが今回いちばん重要な観測である。**このモデルの action 予測は画像の
解像度にほとんど依存していない。** 256 と 128 で学習曲線が重なる。

§27.1 の仮説 1（画像スロットを 5→2 に減らしても loss が 12 点すべて完全一致）
と同じ性質である。画像入力を変えても loss が動かない。

だから **解像度差は §27 の失敗の原因になりようがなかった。** モデルが
そもそも解像度を見ていないのだから、揃えても揃えなくても同じである。
§27.2 で「残る説明」としたのは筋の悪い推測だった。

#### 「ベースは収束済み」という読みも外れ

`init_type` は既定 None で、peft の `init_lora_weights=True`（B をゼロ初期化）
になる（`lerobot/policies/pretrained.py:374`、`configs/default.py:92`）。
したがって **step 0 の出力はベースそのもの**であり、loss 0.93 は
**ベースモデル自身のこのデータでの loss** である。収束済みなら初手から
底のはずで、そうなっていない。

#### loss は 3 分の 1 になり、success は 45 pt 落ちた

これが本質である。

| | 学習 loss | 公開 4 タスク success |
|---|---|---|
| ベース（step 0） | 0.93 | 77.5% |
| step 5,000 | 0.30 付近 | **32.5%** |

**学習は loss を確実に下げている。その loss は success と逆を向いている。**
最適化している量が、評価している量と一致していない。

同じラウンドで観測した付随証拠がこれと整合する。

- `cartesian_path_length` が 1.046 → 0.853（**−18%**）
- 最終エピソードが 300 step を使い切る（base は 87 step で終了）

**action が小さくなっている。** 条件付き平均へ潰れる（mean collapse）と
MSE 系の loss は下がるが、実行すると動きが足りずタスクが終わらない。
断定はできないが、3 つの観測がすべてこの向きを指している。
...
|---|---|---|
| 1〜5 | §27.1 の 5 つ | ✗ |
| 6 | 解像度差 | ✗（§34。しかも **モデルは解像度を見ていない**） |
| 7 | ベースが収束済み | ✗（loss 0.93 スタート） |

**ハイパーパラメータの問題ではない。** lr を下げれば loss の下がり方が
遅くなるだけで、下がる方向が success と逆である以上、どこかで同じ壊れ方に
到達する。`train_expert_only=false` も、より多くのパラメータを同じ
間違った方向へ動かすだけである。

`tools/train_lora.sh` / `tools/merge_lora.py` / `tools/lowres_transform.py` は
残す。**再開するなら、まず「この loss を下げると success が上がる」ことを
小規模で示すこと。** それが示せない限り、何 step 回しても同じである。

15,000 step の run は完走させてよい（10,000 / 15,000 の checkpoint は無料）。
ただし §27 で 15,000 は 5,000 より良かったものの base より 32.5 pt 低かった。
**期待はしない。**

### 34.7 原因はこれ: 損失の 78% がゼロ埋め次元である

「loss が下がるのに success が落ちる」の答えが実装にあった。
**§34.6 の打ち切り判断は取り消す。**

#### 何が起きているか

SmolVLA は action を `max_action_dim`（既定 **32**）までゼロ埋めする。
PARC の action は **7 次元**なので、**25 次元がゼロ埋め**である。

```python
# modeling_smolvla.py
prepare_action      : actions = pad_vector(batch[ACTION], 32)   # 7 実 + 25 ゼロ
VLAFlowMatching.forward
    x_t    = t * noise + (1 - t) * actions
    u_t    = noise - actions
    losses = F.mse_loss(u_t, v_t, reduction="none")   # (B, chunk, 32)
SmolVLAPolicy.forward
    losses = losses[:, :, : self.config.max_action_dim]   # ← 32 で切る = no-op
    loss   = losses.mean()                                # ← 32 次元の平均
```

`# Remove padding` というコメントが付いているが、`max_action_dim` は
ゼロ埋め**後**の次元なので**この行は何も削っていない**。

ゼロ埋め次元では `actions_d = 0` なので

    x_t,d = t * noise_d          u_t,d = noise_d - 0 = x_t,d / t

**目標値が入力から決定的に復元できる。** 時刻 t もネットワークに入って
いるので、これは自明に学習できる関数であり、タスクとも画像とも関係ない。

つまり **損失の 25/32 が、タスク無関係で簡単な項**である。
実測では寄与の **94%** がここだった（下記）。

#### 実測した（2026-08-10、dim7 run の起動直後）

総和からの逆算ではなく、`[dimloss]` で直接測った**ベースモデルの内訳**。

| | 損失 | 32 次元平均への寄与 |
|---|---|---|
| 本物の 7 次元 | **0.2522** | 0.055（6%） |
| ゼロ埋め 25 次元 | **1.0951** | 0.855（**94%**） |
| 32 次元平均 | **0.9107** | — |

**勾配の 94% がゼロ埋め次元へ向いていた。** 当初の見積もりは 78% だったが、
実際はもっと極端である。ゼロ埋め次元の 1.0951 は `E[noise²] ≈ 1` とほぼ
一致し、**ベースはそこを予測していない**（出力 ≈ 0）ことを裏付ける。

またこの 0.9107 は §27 の step 100 の loss 0.910 とほぼ一致する。
別 run・別プロセスで同じ値が出たので、パイプラインの一致も確認できた。

**当初の逆算は本物の次元を L_real ≈ 0.68 と見積もったが、実測は 0.2522
だった。** ベースは実次元については十分よく学習できている。構造の読み
（損失がゼロ埋めに支配されている）は当たっていたが、内訳の数字は外していた。

なお総損失が 0.93 → 0.30 に落ちたぶんが、ゼロ埋め次元の改善だけで
説明できるのか、実次元の悪化を伴うのかは、総和からは決められない
（ゼロ埋めが 1.095 → 0.32 なら実次元は据え置きでも辻褄が合う）。
**dim7 run では実次元だけを直接追える。**

#### 過去の「画像が効かない」も同じ理由で説明が付く

| 観測 | これまでの解釈 | 統一した説明 |
|---|---|---|
| 画像スロット 5→2 で loss が 12 桁一致（§27.1） | 「空スロットはマスクされる」 | 損失の主成分が画像に依存しない |
| 画素数 1/4 で loss が 2% しか動かない（§34.5） | 「モデルは解像度を見ていない」 | 同上 |

どちらも「損失の 78% が画像と無関係な項だから」で説明できる。
**モデルが画像を見ていないのではなく、損失が見ていなかった。**

#### なぜベースは平気で LoRA が壊れるのか

ベースは全パラメータを長時間学習しているので、ゼロ埋め項に容量を取られても
本物の次元を学べる。**rank 8 の LoRA は容量が小さい。** 勾配の 78% が向く
簡単な項に容量を使い切り、本物の 7 次元は巻き添えで悪化する。
`action_out_proj` は 32 次元を一括で出すので、ゼロ埋め次元を合わせに行けば
実次元の出力も動く。

### 34.8 仮説 8: 損失を本物の次元に絞る

`tools/action_dim_loss.py` を追加した。`VLAFlowMatching.forward` の戻り値から
ゼロ埋め次元を落とし、残す次元を `32/7` 倍する（`SmolVLAPolicy.forward` が
32 次元で平均するので、こうすると「7 次元だけの平均」と同じ値・同じ勾配
スケールになる。**実効学習率を変えない**ため、効果の切り分けができる）。

```bash
PARC_LORA_ACTION_DIM=7 PARC_LORA_LOWRES=0 PARC_LORA_TAG=dim7 \
  PARC_LORA_STEPS=5000 CUDA_VISIBLE_DEVICES=2 \
  nohup bash tools/train_lora.sh > "logs/train_dim7_$$.log" 2>&1 &
```

解像度の劣化とは独立に切れる（`PARC_LORA_LOWRES=0`）。§34 で解像度は
効かないと分かったので、まず単独で試すのが筋である。

**最初の 1 分で仮説が検証できる。** 起動直後に出る `[dimloss]` 行が、
ベースモデルの損失内訳をそのまま表示する。

```text
[dimloss] 損失を先頭 7 次元に限定する（ゼロ埋めの 25 次元を勾配から外す）
[dimloss] 本物の 7 次元: 0.68xx   ゼロ埋め次元: 0.9xxx   （素の 32 次元平均なら 0.9xxx、うちゼロ埋めが 78%）
```

→ **実測は 94% だった**（§34.7）。仮説 8 の前提は確認済みである。

`PARC_LORA_ACTION_DIM=7` を付けると、学習ログの `loss:` は
**本物の 7 次元だけの平均**になる（マスク後に 32/7 倍しているため）。
0.25 付近から始まるので、**過去の run の 0.91 とは比較できない。**
比較すべきはこの run 内での 0.2522 からの下がり方である。

`tests/test_action_dim_loss.py` が挙動を固定している（6 件）。ゼロ埋め次元へ
勾配が流れないこと、残した次元のスケールが変わらないことを検証する。

判定はこれまでどおり §26 のゲートで、**同じラウンドに base を置く**こと。
`PARC_LORA_STEPS=5000` にしてあるのは、§27 / §34 とも損傷が step 5,000 で
既に出ていたためである。ここで base 付近に居なければ、この仮説も外れである。

---

## 35. 仮説 8 は当たった。ただし改善もしない

2026-08-10、yamabuki、公開 4 タスク × 10 エピソード・300 step・seed 42・EGL・
GPU 1。`PARC_LORA_ACTION_DIM=7`・`PARC_LORA_LOWRES=0`・step 5,000。

| 条件 | success | collision | cartesian | jerk | sparc |
|---|---|---|---|---|---|
| base | 0.800 | 0.075 | 1.043 | 4.206 | −2.438 |
| dim7 | **0.800** | 0.100 | 1.076 | 4.417 | −2.664 |
| 差 | **±0.000** | +0.025 | +3% | +5% | — |

### 35.1 破壊は止まった（原因は確定）

| 回 | 学習の条件 | step 5,000 の base 比 |
|---|---|---|
| §27 | 素の lerobot（256） | **−37.5 pt** |
| §34 | 解像度を 128 に揃えた | **−45 pt** |
| **§35** | **損失を実 7 次元に限定** | **±0 pt** |

**ゼロ埋め次元を勾配から外しただけで、3 回続いた破壊が完全に止まった。**
§27 で 5 仮説、§34 で解像度を潰したあとに残っていた謎の答えがこれである。

付随する証拠も裏返った。§34 で見えた mean collapse の兆候（`cartesian` −18%、
300 step 使い切り）が消え、今回は `cartesian` +3% でむしろ base より少し長い。
**動かなくなる現象は、ゼロ埋め次元へ勾配を流していたことの結果だった。**

### 35.2 しかし改善もしない

success は完全に同値である。collision +2.5 pt と jerk +5% はどちらも
ぶれの範囲（§32.1: jerk は 15% 未満を読まない、collision は 10 pt 未満を
読まない）。タスク別も 90/70/90/70 対 80/60/100/80 で、上下して打ち消し合う。

**実次元の損失は半分になったのに（0.2522 → 0.1228）、成功率は動かない。**

### 35.3 これで追加学習の線は閉じる（今度は別の理由で）

これまでは「壊れるから」閉じようとしていた。今回それは解決した。
それでも閉じるべきなのは、**このデータに伸びしろが無い**からである。

- ベースの実次元損失は **0.2522**。既にこのデータをよく説明できている
- LoRA はそれを **0.1228** まで下げた。学習は確かに進んでいる
- **それでも公開 4 タスクの成功率は 1 pt も動かない**

ベースが既に解けるデータをさらによく当てはめても、**解けないタスクへの
汎化は増えない**。採点セットの 6/8 は「ベースが知らない摂動」で失敗して
おり、その条件のデータは手元に無い（§25.2: `lerobot/libero_plus` は
ベース自身の学習データ）。

**必要なのは学習の設定ではなく、ベースが解けていない条件のデータである。**
それが無い以上、この軸で得るものは無い。

### 35.4 残しておくもの

`PARC_LORA_ACTION_DIM=7` は**追加学習を再開するときの必須設定**である。
これ無しでは何をやっても壊れる。`tools/train_lora.sh` の既定は 0 のままに
してあるが、**再開する人は必ず 7 を付けること。**

step 15,000 まで回して再評価する余地は残っている（GPU は空いており、
人の手間もほぼ無い）。ただし実次元の損失が半分になって成功率が動かない
以上、**さらに下げても同じだと見ている。**
