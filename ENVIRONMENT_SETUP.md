...
採点で実際に走ったタスクは、採点ログ中のパイプライン出力
（`libero_t1 から N タスクを読み込み` と `タスク評価完了: <名前> — 成功率 X%`）
を見れば分かる。まだ確認していない。

### 22.3 スコアは成功率の単純平均である

`overall_score` は重み付き複合値ではない。`pipeline/scorer.py:102,130` が
...
と定義しており、条件分岐は無い。`pipeline/cli.py:199` が印字する
`総合スコア` はこの値そのままである。`total_score_config.json` /
`normalization_config.json` は SPARC 等の補助指標の正規化にしか使われず、
`overall_score` には影響しない。

したがって採点の 0.043 は **タスクごとの成功率をタスク数で平均した 4.3%** である。
ローカルの公開 4 タスク 82.5%（§23）との差はチューニングでは説明できない大きさで、
汎化の問題として扱う必要がある。

0.043 をどう分解できるか（何タスク × 何エピソードか）は、採点ログの
レイテンシ行が 7 本しか見えていない一方で 0.043 が 1/23 に近いという
食い違いがあり、未確定である。§22.2 の方法で確認する。
