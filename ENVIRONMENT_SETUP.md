...
つまり **損失の 25/32 = 78% が、タスク無関係で簡単な項**である。

#### 数字が合う

初期 loss 0.93 から逆算する。ベースがゼロ埋め次元を予測していない
（出力 ≈ 0）なら、その寄与は `E[noise²] = 1` ずつで 25/32 = 0.78。
残りが実次元なので `7·L_real/32 = 0.15` → **L_real ≈ 0.68**。

step 2,000 の 0.30 で、ゼロ埋め次元を学習し終えた（残差 0.05 程度）と置くと
`25·0.05/32 = 0.04`、`7·L_real'/32 = 0.26` → **L_real' ≈ 1.19**。

**総損失は 3 分の 1 になり、本物の次元の損失は 2 倍近くに悪化している。**
success が 45 pt 落ちたことと整合する。（この分解は総和からの逆算であって
実測ではない。下の `[dimloss]` ログで直接測れるようにした。）
...
- **ゼロ埋めが 70% 以上** → 上の逆算どおり。この仮説で正しい
- **ゼロ埋めが小さい** → 逆算が外れ。別の説明を探す
