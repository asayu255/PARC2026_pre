...
## 29. temporal ensembling を実装した（既定 off・測定待ち）

§28.2 の一手を `submission/policy_server.py` に入れた。まだ wakaba で
評価していないので、**既定は無効**にしてある。3 回目の採点 0.077 を出した
構成をそのまま残すためで、A/B（下記）で確認してから既定を切り替える。
`FLIP_IMAGES_180` も `N_ACTION_EXEC` も同じ順序で決めた。
