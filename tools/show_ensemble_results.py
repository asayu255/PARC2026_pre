...
def main():
    ...
    print()
    print("  jerk がいちばん安定だが、それでも 4 タスク x 10 ep では同一条件の")
    print("  再測定が 4.8% 動いた（h=8 を 3 ラウンド: 4.508 / 4.559 / 4.344）。")
    print("  **単発のラウンドで信用してよいのは 15% 以上の差だけ**である。")
    print("  それ未満は別ラウンドで再現するまで結論にしないこと（§32.1）。")
    print()
