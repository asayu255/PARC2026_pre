...
#:
#: 順番に意味がある。PARC_OFT_TTA=N は先頭 N 個を使うので、少ない視点でも
#: 既定 (0.90) を中心に対称へ広がるよう並べてある。1 視点なら本家と同一。
#:
#: 8 まで用意してあるのは、TTA が採点で効いた唯一の軸だからである
#: （0.293 -> 0.304、公開タスクでも collision 半減）。1 リクエストの制限は
#: 10 秒で、4 視点の実測が 0.644 秒。8 視点でもまだ 7 倍以上の余裕がある。
#:
#: **1.00 は入れない。** 倍率 1.00 のクロップは恒等変換で（`_center_crop` の
#: 定義そのもの。テストでも固定してある）、「クロップしない」条件と同値である。
#: それは `PARC_OFT_CENTER_CROP=0` として単独で測って**悪化した**条件なので、
#: 平均に混ぜる理由が無い。
#:
#: 実際、旧版はここに 1.00 が 4 番目に入っていた。採点 8 回目の `tta4`
#: （= 平均の 1/4 がクロップ無し）は 0.271 で、`tta2` の 0.304 を下回った。
#: 視点を増やすこと自体が悪いのではなく、悪いと分かっている視点を混ぜていた。
